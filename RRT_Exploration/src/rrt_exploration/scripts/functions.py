...
class robot:
    ...
    def __init__(self, name): # robot class construct function                self is      shili
        self.assigned_point = []
        self.name = name
        self.global_frame = rospy.get_param('~global_frame', '/map')
        self.robot_frame = rospy.get_param('~robot_frame', 'base_link')
        self.plan_service = rospy.get_param(
            '~plan_service', '/move_base/GlobalPlanner/make_plan') # navigation
        ...
        while cond == 0:
            try:
                ...
                cond == 0
        self.position = array([trans[0], trans[1]]) # robot position to global frame
        self.assigned_point = self.position
        self.client = actionlib.SimpleActionClient(
            self.name+'/move_base', MoveBaseAction)
        self.client.wait_for_server()
        robot.goal.target_pose.header.frame_id = self.global_frame #todo: in Ubuntu18.04 this is a bug, look at sendGoal()
    ...
    def sendGoal(self, point):
        ...
        robot.goal.target_pose.header.frame_id = self.name[1:] + '/map' #todo:add this to Debug
...
def index_of_point(mapData, Xp): # index form
...
def point_of_index(mapData, i): # point form
