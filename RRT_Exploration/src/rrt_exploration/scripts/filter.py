...
## can shu de han yi can kao roswiki de rrt_exploration
    ...
    # For example, if you have three robots with the following namespaces: "/robot_1","/robot_2", and "/robot_3", then you have to set this parameter to "/robot_" 
    namespace = rospy.get_param('~namespace', '')
    namespace_init_count = rospy.get_param('namespace_init_count', 1)
    rateHz = rospy.get_param('~rate', 100)
    global_costmap_topic = rospy.get_param( # The costmaps are used to delete invalid points
        '~global_costmap_topic', '/move_base/global_costmap/costmap')
    robot_frame = rospy.get_param('~robot_frame', 'base_link')

    litraIndx = len(namespace)# then you have to set this parameter to "/robot_" 
    rate = rospy.Rate(rateHz)
# -------------------------------------------
# The map is used to know which points are no longer frontier points (old points). 
...
# The costmaps are used to delete invalid points.
    if len(namespace) > 0:
        for i in range(0, n_robots): # this topic is publiced by robot navigation stack
...
# wait if any of robots' global costmap map is not received yet
    ...
    if len(namespace) > 0:
        for i in range(0, n_robots):
            tfLisn.waitForTransform(global_frame[1:], namespace+str( # [1:] from 1 start no "/"
    ...
    rospy.Subscriber(goals_topic, PointStamped, callback=callBack,
                     callback_args=[tfLisn, global_frame[1:]]) # save frontiers
    pub = rospy.Publisher('frontiers', Marker, queue_size=10)
    pub2 = rospy.Publisher('centroids', Marker, queue_size=10)
    filterpub = rospy.Publisher('filtered_points', PointArray, queue_size=10) # to assigner
        ...
        while z < len(centroids):
            ...
            for i in range(0, n_robots):
                ...
                transformedPoint = tfLisn.transformPoint(
                    globalmaps[i].header.frame_id, temppoint) # local frame to globalmaps frame?  costmap?
                x = array([transformedPoint.point.x, transformedPoint.point.y])
                cond = (gridValue(globalmaps[i], x) > threshold) or cond #clearing invalid point 
            if (cond or (informationGain(mapData, [centroids[z][0], centroids[z][1]], info_radius*0.5)) < 0.2): # clearing old frontiers
...
# publishing
        ...
        filterpub.publish(arraypoints) # to assigner
        ...
        pub.publish(points)
        pub2.publish(points_clust) # visual
        rate.sleep()
# -------------------------------------------------------------------------
