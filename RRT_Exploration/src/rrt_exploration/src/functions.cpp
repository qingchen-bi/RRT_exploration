...
rdm::rdm() {i=time(0);} //得到系统时间，作为srand的种子，srand每次中下的随机时间不同，rand得到的随机数就不一样。
float rdm::randomize() { i=i+1;  srand (i);  return float(rand())/float(RAND_MAX);} //除以 RAND_MAX（返回的最大的随机数） 得到0-1.0之间的数。



//Norm function 根号下x^2 + y^2.
...
//Nearest function：在树上找到距离xrand最近的xnearest
...
//Steer function：根据xrand、xnearest、eta 得到xnew。
...
//gridValue function：根据珊格值判断是什么类型的区域
...
for (int c=0;c<stepz;c++){
  xi=Steer(xi,xnew,rez);
   ...
   if (gridValue(mapsub,xi) ==-1){      unk=1;	break;}//只要发现是有unkown的点就停止下来
  }
char out=0;
 xnew=xi;
 if (unk==1){  out=-1;}//有unkown
